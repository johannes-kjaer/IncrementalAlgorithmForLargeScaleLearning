...
def multinormalRandomData(dim, n_samples, c1, c2):
...
def testDataGeneration(c1=3, c2=2):
    data, cov, V, L = multinormalRandomData(2, 1000, c1, c2)
    eVals = np.diag(L)
...
def newGenerateCompleteData(dim, nb_samples, c1, c2, w=None):
    X, cov, V, L = multinormalRandomData(dim, nb_samples, c1, c2)
    if w is None:
