...
def generateRandomData(n, samples, ratio, mean = None, cov = None):
    ...
    if cov == None:
        cov = generateRandomCovMatrix(n) + ratio * np.identity(n,dtype=np.double)

    randomData = np.random.multivariate_normal(mean, cov, samples)
